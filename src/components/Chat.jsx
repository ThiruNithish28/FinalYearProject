  ...
  return (
    <>
      <div className="h-full  flex flex-1 flex-col items-center  p-4 lg:p-8 text-white">
        {/* user query  */}
        <div className="w-[450px] break-words whitespace-pre-wrap bg-gray-btn self-end text-xl font-semibold rounded-xl mr-8 px-4 py-2.5">
          {query}
        </div>
        {/* response  */}
        <div
          dangerouslySetInnerHTML={{ __html: renderedResponse }}
          className="w-full h-full lg:w-[860px] p-4 rounded-xl whitespace-pre-wrap prose prose-invert max-w-none [&_pre]:overflow-x-auto [&_pre]:p-4 [&_pre]:rounded-lg  [&_code]:text-sm [&_code]:block"
        ></div>
      </div>
    </>
  );
