...
const stopWords = [
  "for", "a", "the", "to", "on", "in", "and", "of", "at", "by",
];
const importantShortWords = ["ai", "js", "go", "c", "c++", "sql"];

// Extract hashtags from the keyword generated by Gemini and remove stop words and duplicates
const extractHashtags = (keyword) => {
...
// Get community posts by tags for the chat module
export const getCommunityPosts = async (keyword) => {
  const tags = extractHashtags(keyword);
  if (tags.length === 0) return [];

  const mustHaveTag = tags[0]; // prioritize the first tag
  const { data, error } = await supabase
    ...
    .limit(20); // fetch more initially to allow filtering

  if (error) throw new Error(error.message);

  // Filter to only posts that contain the must-have tag
  const filtered = data.filter((post) => post.tags.includes(mustHaveTag));
